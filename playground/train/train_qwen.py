#!/usr/bin/env python3
"""
Qwen LoRA Training Script for Adaptive Minds

Train a new domain expert with Qwen-2.5-7B-Instruct as base model.

Usage:
    python train_qwen.py --dataset medalpaca/medical_meadow_medical_flashcards --lora-name medical-expert
    python train_qwen.py --dataset squad --lora-name qa-expert --steps 300
"""
...
def format_prompt_qwen(example):
    """Format training example into Qwen chat format."""
    instruction = None
    response = None
    input_text = ""
...
def train_lora_qwen(
    ...
    """Train a LoRA adapter for Qwen-2.5-7B-Instruct."""

    # Paths - using Qwen base model
    base_model_path = "../base_model/qwen-2.5-7B-instruct"
    output_dir = f"../loras/{lora_name}"

    logger.info(f"🚀 Training Qwen LoRA: {lora_name}")
    logger.info(f"📊 Dataset: {dataset_name}")
    logger.info(f"💾 Output: {output_dir}")
    logger.info(f"🔧 Base Model: Qwen-2.5-7B-Instruct")

    # Load model
    logger.info("Loading Qwen-2.5-7B-Instruct base model...")
    ...
    # Add LoRA - Qwen uses different target modules
    ...
    # Format dataset with Qwen chat template
    def formatting_func(examples):
        texts = []
        for i in range(len(examples[list(examples.keys())[0]])):
            example = {key: examples[key][i] for key in examples.keys()}
            formatted = format_prompt_qwen(example)
            if formatted:
    ...
    if os.path.exists("../manage_models.py"):
        ...
        logger.info(f'   python ../manage_models.py add-lora --name "{lora_name}" --path "{output_dir.replace("../", "./")}" --description "Custom trained Qwen expert" --system-prompt "You are an expert in your domain" --keywords custom domain expert')

def main():
    parser = argparse.ArgumentParser(description="Qwen LoRA training for Adaptive Minds")
